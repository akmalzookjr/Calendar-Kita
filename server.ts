...
try {
  ...
  // Add missing columns
  ensureColumn('users', 'accentColor', "TEXT DEFAULT '#10b981'");
...
async function startServer() {
  ...
  app.get("/api/auth/me", authenticate, async (req: any, res) => {
    try {
      ...
      const user = db.prepare(`
        SELECT id, username, name, bio, profileImage, role, isAdmin, themeColor, accentColor, backgroundStyle, 
  ...
  app.get("/api/profile", authenticate, (req: any, res) => {
    const userId = req.user.id;
    const user = db.prepare(`
      SELECT id, username, name, bio, profileImage, role, themeColor, accentColor, backgroundStyle, createdAt, updatedAt 
      FROM users WHERE id = ?
    `).get(userId) as any;
  ...
  app.put("/api/profile", authenticate, upload.single('profileImage'), async (req: any, res) => {
    const userId = req.user.id;
    const { name, bio, password, themeColor, accentColor, backgroundStyle } = req.body;
    ...
    try {
      ...
      if (updates.length > 0) {
        ...
        if (accentColor !== undefined) supabaseUpdates.accentColor = accentColor;
      ...
      broadcast({ type: "USER_UPDATED", payload: { userId, profileImage: updatedUser.profileImage } });
...
app.get("/api/holidays/sync/:year", authenticate, async (req: any, res) => {
  ...
  try {
    ...
    if (!apiSuccess) {
      ...
      if (year === "2026") {
        holidays = [
          ...
          { date: "2026-06-17", localName: "Awal Muharram", name: "Islamic New Year", type: "public_holiday" },
          ...
          { date: "2026-11-08", localName: "Deepavali", name: "Deepavali", type: "public_holiday" },
    ...
    // If using API data, categorize holidays
    let processedHolidays = holidays;
    
    if (apiSuccess) {
      processedHolidays = holidays.map((holiday: any) => {
        // Determine if it's a public holiday or observance
        let type = 'observance'; // default
        
        // Check if it's a public holiday based on various indicators
        const title = holiday.localName?.toLowerCase() || holiday.name?.toLowerCase() || "";
        
        // List of public holiday keywords
        ...
        // Check holiday type from API if available
        if (holiday.type) {
          if (Array.isArray(holiday.type)) {
            if (holiday.type.includes("National holiday") || 
                holiday.type.includes("Public Holiday") ||
                holiday.type.includes("Federal Public Holiday")) {
              type = 'public_holiday';
            }
          } else if (typeof holiday.type === 'string') {
            if (holiday.type.includes("National") || holiday.type.includes("Public")) {
              type = 'public_holiday';
            }
          }
        }

        // If no type from API, check by keywords
        if (type === 'observance') {
          const isPublic = publicHolidayKeywords.some(keyword => title.includes(keyword));
          type = isPublic ? 'public_holiday' : 'observance';
        }
      ...
      // Log counts
      const publicCount = processedHolidays.filter((h: any) => h.type === 'public_holiday').length;
      const observanceCount = processedHolidays.filter((h: any) => h.type === 'observance').length;
      console.log(`Categorized: ${publicCount} public holidays, ${observanceCount} observances`);
    }

    // Insert holidays into local database with type
    ...
    processedHolidays.forEach((holiday: any) => {
    ...
    try {
      const holidayInserts = processedHolidays.map((holiday: any) => ({
    ...
    console.log(`✅ Total ${insertedCount} events synced for ${year}`);
    res.json({ 
...
app.get("/api/events", authenticate, (req: any, res) => {
  try {
    ...
    if (groupIds.length > 0) {
      ...
      events = db.prepare(`
        SELECT DISTINCT e.*, u.profileImage, strftime('%Y-%m-%dT%H:%M:%SZ', e.createdAt) as createdAt, 
      ...
      events = db.prepare(`
        SELECT e.*, u.profileImage, strftime('%Y-%m-%dT%H:%M:%SZ', e.createdAt) as createdAt, 
        (SELECT COUNT(*) FROM comments WHERE eventId = e.id) as commentCount 
        FROM events e 
        LEFT JOIN users u ON e.userId = u.id
        WHERE e.userId = ? 
  ...
  app.post("/api/events", authenticate, async (req: any, res) => {
    ...
    try {
      ...
      const userProfile = db.prepare("SELECT profileImage FROM users WHERE id = ?").get(userId) as any;
  ...
  app.put("/api/events/:id", authenticate, async (req: any, res) => {
    ...
    try {
      ...
      const userProfile = db.prepare("SELECT profileImage FROM users WHERE id = ?").get(event.userId) as any;
